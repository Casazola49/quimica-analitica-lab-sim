...
import { CheckSquare, Square, AlertCircle, HelpCircle, X, ArrowRight, RotateCcw } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-black/90 backdrop-blur-md animate-in fade-in duration-200 select-none font-sans">
      <div className="relative w-full max-w-3xl max-h-[90vh] bg-[#0a0a0a] border border-[#262626] rounded-2xl shadow-2xl flex flex-col overflow-hidden text-white">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 text-xs">
          <div className="p-3.5 bg-[#141414] border-l-4 border-l-[#dc2626] border-y border-r border-[#242424] rounded-xl flex items-start justify-between gap-2 text-[#cccccc] shadow">
            <div className="flex items-start gap-2">
              <HelpCircle size={16} className="shrink-0 mt-0.5 text-[#ef4444]" />
            ...
            <button
              ...
              className="shrink-0 flex items-center gap-1 text-[10px] px-2.5 py-1 bg-[#1a1a1a] hover:bg-[#282828] text-white rounded-lg border border-[#333333] cursor-pointer"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between pb-1.5 border-b border-[#222222]">
                <span className="font-bold text-white uppercase text-[11px] tracking-wider font-serif">
                  ...
                  return (
                    <div
                      ...
                      className={`p-2.5 rounded-xl border cursor-pointer transition-all flex items-start gap-2.5 ${
                        isChecked
                          ? 'bg-[#1c0808] border-[#dc2626] text-white shadow-md shadow-red-950/40'
                          : 'bg-[#121212] hover:bg-[#1a1a1a] border-[#222222] text-[#888888] hover:text-white'
                      }`}
                    >
                      <button type="button" className="mt-0.5 shrink-0 text-[#dc2626]">
                        {isChecked ? <CheckSquare size={16} className="text-[#dc2626] fill-[#1c0808]" /> : <Square size={16} className="text-[#444444]" />}
                      </button>
        ...
        <div className="p-4 bg-[#121212] border-t border-[#222222] flex items-center justify-between gap-3">
          <button
            onClick={() => {
              const autoSet = new Set(baseItems.filter((i) => i.isRequired).map((i) => i.id));
              setSelectedIds(autoSet);
            }}
            className="text-[11px] text-[#aaaaaa] hover:text-white underline font-medium cursor-pointer"
          >
          ...
          <div className="flex items-center gap-2 font-serif">
            <button
              onClick={onClose}
              className="px-4 py-1.5 bg-[#171717] hover:bg-[#222222] border border-[#333333] text-[#cccccc] hover:text-white rounded-xl text-xs font-semibold cursor-pointer transition-all"
            >
              Cerrar
            </button>
            <button
              onClick={handleVerify}
              className="px-5 py-2 bg-[#dc2626] hover:bg-[#b91c1c] active:scale-95 text-white rounded-xl text-xs font-extrabold shadow-lg shadow-red-950/50 transition-all cursor-pointer uppercase tracking-wider"
            >
        ...
        {/* NOTIFICACIÓN CENTRADA AL MEDIO (CON SELLO HANKO) */}
        {showFeedbackModal && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-md animate-in fade-in zoom-in-95 duration-150">
            <div className="w-full max-w-md bg-[#0d0d0d] border-2 border-[#dc2626] rounded-2xl p-6 shadow-2xl space-y-4 text-center">
              {isPerfect ? (
                <>
                  <div className="flex justify-center">
                    <HankoSeal size="lg" variant="stamp" />
                  </div>
                  <div>
                    <h3 className="text-base font-extrabold text-white uppercase tracking-wider font-serif">
                      ¡Solicitud Aprobada por el Ayudante!
                    </h3>
                    <p className="text-xs text-[#cccccc] mt-1.5 leading-relaxed font-sans">
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-[#dc2626] hover:bg-[#b91c1c] active:scale-95 text-white font-extrabold rounded-xl shadow-xl flex items-center justify-center gap-2 text-xs transition-all cursor-pointer uppercase tracking-wider font-serif"
                  >
                ...
                <>
                  <div className="w-14 h-14 mx-auto rounded-full bg-[#2a0505] border-2 border-[#dc2626] flex items-center justify-center text-[#ef4444]">
                    <AlertCircle size={32} />
                  </div>
                  <div>
                    <h3 className="text-base font-extrabold text-white uppercase tracking-wider font-serif">
                  ...
                  <div className="max-h-48 overflow-y-auto text-left text-[11px] space-y-2.5 p-3.5 bg-[#050505] border border-[#222222] rounded-xl font-sans">
                    {missingRequired.length > 0 && (
                      <div>
                        <strong className="text-[#ef4444]">Faltan ({missingRequired.length}) necesarios:</strong>
                        <ul className="list-disc list-inside text-[#cccccc] mt-0.5 space-y-0.5">
                    ...
                    {chosenDistractors.length > 0 && (
                      <div className="pt-1.5 border-t border-[#222222]">
                        <strong className="text-[#ff4444]">Distractores innecesarios ({chosenDistractors.length}):</strong>
                        <ul className="list-disc list-inside text-[#cccccc] mt-0.5 space-y-0.5">
                          {chosenDistractors.map((d) => (
                            <li key={d.id} className="text-[#ff8888] truncate">{d.name} ({d.reason})</li>
                          ))}
                        </ul>
                      </div>
                    )}
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-[#171717] hover:bg-[#242424] active:scale-95 text-white font-bold rounded-xl text-xs transition-all cursor-pointer border border-[#333333]"
                  >
