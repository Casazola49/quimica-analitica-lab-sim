...
interface LabNotebookProps {
  ...
  finalVolume: number;
  sampleMass: number; // P4: masa KHP (~0.21 g). P5: muestra sulfatos (~0.50 g). P7: alícuota 25 mL
...
export const LabNotebook: React.FC<LabNotebookProps> = ({
  ...
  const handleEvaluate = () => {
    const data: LabNotebookData = {
      ...
      titrantName: practiceNumber === 5 ? 'BaCl2 5%' : 'Hidróxido de Sodio',
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 text-xs text-slate-200 min-h-0">
        {activeTab === 'data' ? (
          <>
            ...
            {/* FORMULARIO ESPECÍFICO SEGÚN PRÁCTICA */}
            {practiceNumber === 5 ? (
              /* Campos para Gravimetría P5 */
              <div className="space-y-3">
                ...
                {/* Porcentaje de Sulfato calculado por alumno */}
              ...
              /* Campos para Volumetría P4 y P7 */
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {practiceNumber === 7 ? (
                  <div className="space-y-1">
