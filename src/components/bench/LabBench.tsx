...
export const LabBench: React.FC<LabBenchProps> = ({
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-between p-4 lab-bench-bg rounded-2xl border border-slate-700/80 shadow-2xl relative overflow-hidden">
      {/* Selector de Velocidad de la Llave */}
      <div className="w-full flex items-center justify-between z-10 px-3">
        <div className="flex items-center gap-2 bg-slate-800/90 border border-slate-700 px-3 py-1.5 rounded-xl shadow">
          <span className="text-xs text-slate-300 font-medium">Modo de Flujo:</span>
          <button
        ...
        {equilibrium.endpointQuality !== 'none' && (
          <div
            className={`px-3 py-1 rounded-xl text-xs font-bold border shadow animate-in fade-in ${
          ...
          >
            {equilibrium.endpointQuality === 'perfect'
              ? '🌸 ¡Primer viraje rosa tenue persistente! (Punto final alcanzado)'
              : '⚠️ Solución sobretitulada (Exceso fucsia de NaOH)'}
          </div>
        )}
      </div>
